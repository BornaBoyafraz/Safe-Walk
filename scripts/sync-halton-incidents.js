...
// Halton Regional Police open data — ArcGIS Feature Service
// Source: https://data.haltonpolice.ca/datasets/major-crime-indicators
const BASE_URL =
  'https://services.arcgis.com/7w0OlN44ZUxNABmp/arcgis/rest/services/' +
  'Halton_MCI/FeatureServer/0/query';
...
async function fetchPage(offset) {
  ...
  if (!res.ok) {
    throw new Error(`Halton Police API returned ${res.status}: ${await res.text()}`);
  }
  return res.json();
}
...
async function syncAll() {
  ...
  const upsert = db.prepare(`
    ...
    VALUES (@event_id, @lat, @lng, @category, @offence, @occurred_at, @reported_at, @premise_type, @neighbourhood, 'halton_police')
  ...
  console.log('Fetching Halton Regional Police Major Crime Indicators...');
  ...
  while (true) {
    ...
    try {
      ...
      console.error('Check that the Halton Police ArcGIS endpoint is publicly accessible.');
      process.exit(1);
    }
    ...
    const batch = db.transaction((features) => {
      for (const f of features) {
        ...
        const eventId = `halton_${a.OBJECTID ?? a.EVENT_UNIQUE_ID ?? a.ObjectId}`;
  ...
  console.log(`Done. ${inserted} Halton incidents upserted, ${skipped} skipped.`);
  const count = db.prepare("SELECT COUNT(*) as n FROM incidents WHERE source='halton_police'").get();
  console.log(`Total Halton incidents in database: ${count.n}`);
  db.close();
}

syncAll().catch(err => {
  console.error('Halton sync failed:', err.message);
  process.exit(1);
});
