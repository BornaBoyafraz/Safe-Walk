...
// York Regional Police open data — ArcGIS Feature Service
// Source: https://data.yrp.ca/datasets/major-crime-indicators
const BASE_URL =
  'https://services.arcgis.com/nMEHxMZMOSfBjpPL/arcgis/rest/services/' +
  'YRP_MCI/FeatureServer/0/query';
...
async function fetchPage(offset) {
  ...
  if (!res.ok) {
    throw new Error(`York Police API returned ${res.status}: ${await res.text()}`);
  }
  return res.json();
}
...
async function syncAll() {
  ...
  const upsert = db.prepare(`
    ...
    VALUES (@event_id, @lat, @lng, @category, @offence, @occurred_at, @reported_at, @premise_type, @neighbourhood, 'york_police')
  ...
  console.log('Fetching York Regional Police Major Crime Indicators...');
  ...
  while (true) {
    ...
    try {
      ...
      console.error('Check that the York Police ArcGIS endpoint is publicly accessible.');
      process.exit(1);
    }
    ...
    const batch = db.transaction((features) => {
      for (const f of features) {
        ...
        const eventId = `york_${a.OBJECTID ?? a.EVENT_UNIQUE_ID ?? a.ObjectId}`;
  ...
  console.log(`Done. ${inserted} York incidents upserted, ${skipped} skipped.`);
  const count = db.prepare("SELECT COUNT(*) as n FROM incidents WHERE source='york_police'").get();
  console.log(`Total York incidents in database: ${count.n}`);
  db.close();
}

syncAll().catch(err => {
  console.error('York sync failed:', err.message);
  process.exit(1);
});
